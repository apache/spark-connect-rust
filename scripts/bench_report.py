...
def render(ref_path: str, ours_path: str) -> int:
    ...
    p("## Benchmark results\n")
    p(f"- Reference client: pyspark {ref['pyspark_version']} ({ref['arch']}, py{ref['python']}) at `{ref['client_path']}`")
    p(f"- Rust client: pyspark-client-rust {ours['pyspark_version']} ({ours['arch']}, py{ours['python']})")
    p(f"- Both clients → same Spark Connect server `{ref['remote']}`; iters={ref['iters']}, warmup={ref['warmup']}\n")

    # Transport sweep
    rt, ot = ref["results"].get("transport", {}), ours["results"].get("transport", {})
    if rt and ot:
        p("### Transport: payload-size sweep (df.collect)\n")
        p("| payload | rows | ref p50 (ms) | ours p50 (ms) | speedup | ref GB/s | ours GB/s | ref Mrows/s | ours Mrows/s |")
        p("|---|--:|--:|--:|--:|--:|--:|--:|--:|")
        for k in rt:
            r, o = rt[k], ot.get(k, {})
            if not o:
            ...
            p(f"| {k} | {r['rows']:,} | {_ms(r['latency']['p50_s'])} | {_ms(o['latency']['p50_s'])} | "
        ...
        for k in rt:
            ...
            if not o:
                continue
            p(f"| {k} | {_ms(r['latency']['p99_s'])} | {_ms(o['latency']['p99_s'])} | "
        ...
        p(f"| client CPU / GB decoded | {_f(rb.get('client_cpu_per_gb'),' s')} | {_f(ob.get('client_cpu_per_gb'),' s')} |")
        p(f"| server (JVM) CPU / GB | {_f(rb.get('server_cpu_per_gb'),' s')} | {_f(ob.get('server_cpu_per_gb'),' s')} |")
        p(f"| client peak RSS | {_f(rb.get('peak_rss_mb'),' MB')} | {_f(ob.get('peak_rss_mb'),' MB')} |")
        p("")
    ...
    if rr and orr:
        ...
        for k in rr:
            ...
            if not o:
                continue
            sp = r["latency"]["p50_s"] / o["latency"]["p50_s"]
            p(f"| {int(k):,} | {_ms(r['latency']['p50_s'])} | {_ms(o['latency']['p50_s'])} | {sp:.2f}x | {o['rows_per_s_p50']/1e6:.2f} |")
        p("")
    ...
    if rc and oc and "error" not in rc and "error" not in oc:
        ...
        p("|---|--:|--:|--:|")
        for phase, key in [("import pyspark", "import"), ("build session", "session"),
                           ("first query", "first_query"), ("process total (incl. spawn)", "process_total")]:
    ...
    if rcc and occ:
        ...
        for k in rcc:
            ...
            p(f"| {k} | {r['agg_gb_per_s']:.3f} | {o['agg_gb_per_s']:.3f} | {_ms(rp99)} | {_ms(op99)} |")
        p("")
