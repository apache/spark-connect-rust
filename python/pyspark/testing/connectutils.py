...
# Import test utilities - these should come from the upstream Apache Spark
# For now, use try/except to handle cases where they're not available
try:
    ...
    # Stub implementations for when utils is not available
    have_pandas = False
    connect_requirement_message = "Spark Connect not available"
    should_test_connect = False
    PySparkErrorTestUtils = object
    PySparkBaseTestCase = object
...
class ReusedConnectTestCase(PySparkBaseTestCase, SQLTestUtils, PySparkErrorTestUtils):
    ...
    def setUpClass(cls):
        ...
        if not is_remote_only():
            # For pure-client mode (our Rust-backed client), _instantiatedSession may be None
            # So check for that before trying to access _sc
