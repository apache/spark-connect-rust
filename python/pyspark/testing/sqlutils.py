...
# For Spark Connect (or other environments without SPARK_HOME), we make SPARK_HOME optional
try:
    from pyspark.find_spark_home import _find_spark_home
    SPARK_HOME = _find_spark_home()
except Exception:
    # If SPARK_HOME cannot be found, set it to None. This is acceptable for
    # Spark Connect clients which don't need JARs or build artifacts.
    SPARK_HOME = None
...
def search_jar(project_relative_path, sbt_jar_name_prefix, mvn_jar_name_prefix, return_first=False):
    ...
    if SPARK_HOME is None:
        return None  # No SPARK_HOME available (e.g., in Spark Connect mode)
...
def get_sbt_runtime_classpath(project_relative_path, project_name_map):
    ...
    if SPARK_HOME is None:
        return None  # No SPARK_HOME available
    ...
    try:
        ...
        # SBT's `show dependencyClasspath` emits lines like:
        #   [info] * Attributed(/path/to/file.jar)
        # The Attributed(...) format is part of SBT's core attributed path API and has
        # been stable across all SBT versions used by Spark. If SBT changes this format,
        # update the regex below to match the new output.
        jar_paths = []
...
def read_classpath(project_relative_path, project_name_map=None):
    ...
    For SBT builds (when classpath.txt doesn't exist), this function will query SBT
    for the runtime classpath.

    Args:
        project_relative_path: Relative path from SPARK_HOME to the project directory
        project_name_map: Optional dict mapping project paths to SBT project names.
                         Required if classpath.txt doesn't exist and SBT query is needed.

    Returns:
        Comma-separated string of JAR paths

    Raises:
        RuntimeError: If the classpath cannot be determined
    ...
    """
    if SPARK_HOME is None:
        raise RuntimeError("SPARK_HOME not available (running in Spark Connect mode?)")
    project_full_path = os.path.join(SPARK_HOME, project_relative_path)
    ...
    # Fall back to querying SBT for runtime classpath (SBT builds)
    if project_name_map is not None:
        sbt_classpath = get_sbt_runtime_classpath(project_relative_path, project_name_map)
        if sbt_classpath:
            return sbt_classpath

    # If both methods fail, raise an error
    raise RuntimeError(
        f"Classpath file not found at {classpath_file} and could not query SBT. "
