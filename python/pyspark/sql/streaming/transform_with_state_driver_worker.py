...
import json
from typing import Any, Iterator, TYPE_CHECKING, Optional, IO

# Try to import server-only dependencies
try:
    from pyspark.worker_util import get_sock_file_to_executor, check_python_version
    ...
    pickle_ser: Optional[CPickleSerializer] = CPickleSerializer()
    utf8_deserializer: Optional[UTF8Deserializer] = UTF8Deserializer()
except ImportError:
    # These imports are only needed when this module is actually executed as a driver worker
    # (server-side only). Allow import of this module for inspection/documentation purposes
    # even if the full worker infrastructure isn't available.
    _WORKER_AVAILABLE = False
    pickle_ser = None
    utf8_deserializer = None
...
def main(infile: IO, outfile: IO) -> None:
    if not _WORKER_AVAILABLE:
        raise RuntimeError(
            "Driver worker dependencies are not available. "
            "This module is meant to be executed on the Spark server side only."
        )
