...
# Import classic-only dependencies with guards (DStream requires RDD at runtime)
try:
    from pyspark.core.rdd import portable_hash, RDD
except ImportError:
    # In Connect-only mode, RDD and portable_hash are not available
    # Provide fallback implementations
    class RDD:  # type: ignore
        """Fallback RDD class for type annotations when pyspark.core.rdd is not available."""
        def __class_getitem__(cls, item: Any) -> Any:
            """Support subscripting for type annotations (e.g., RDD[T])."""
            return cls

    def portable_hash(x: Any) -> int:  # type: ignore
        """Fallback hash function when pyspark.core.rdd is not available."""
        return hash(x)

try:
    from pyspark.resultiterable import ResultIterable
except ImportError:
    # In Connect-only mode, ResultIterable may not be available
    class ResultIterable:  # type: ignore
        """Fallback ResultIterable when pyspark.resultiterable is not available."""
