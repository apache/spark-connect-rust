...
# Import SparkConf (available in this drop-in)
from pyspark import SparkConf

# Import classic-only dependencies with guards (DStream requires RDD and SparkContext)
try:
    from pyspark import RDD
except ImportError:
    # In Connect-only mode, RDD may not be available
    class RDD:  # type: ignore
        """Fallback RDD class for type annotations when RDD is not available."""
        def __class_getitem__(cls, item: Any) -> Any:
            """Support subscripting for type annotations (e.g., RDD[T])."""
            return cls

try:
    from pyspark.core.context import SparkContext
except ImportError:
    # In Connect-only mode, classic SparkContext is not available
    # Provide a fallback class
    class SparkContext:  # type: ignore
        """Fallback SparkContext for Connect-only environment."""
        pass
